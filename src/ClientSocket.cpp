...
ClientSocket::ClientSocket(ServerConfig& conf, const sockaddr_in& addr) : _conf(conf),  _complete_post_flag(false),  _complete_parse_flag(false), _post_body_flag(false), _response_flag(false), _client_addr(addr) {}
...
bool ClientSocket::handleEpollOutEvent()
{
	...
	try
	{
		...
		if (epoll_ctl(Server::_epoll_fd, EPOLL_CTL_MOD,this->_fd, &ev) == -1) {
			perror("epoll_ctl: mod");
		}
		std::cout << "-----"<<std::endl;
		return (true);
	...
	catch (const ResponseException& e)
	{
		...
		ev.events = EPOLLIN;
		ev.data.fd = this->_fd;
		if (epoll_ctl(Server::_epoll_fd, EPOLL_CTL_DEL,this->_fd, &ev) == -1) {
				return (false);
			}
		if (clposeAndDeleteSocket(Server::_socket) == false)
			return (false);
