...
public:
	CgiSocket(pid_t cgi_pid, int read_fd, int write_fd, std::string& request_body, std::string& response);
	~CgiSocket();
	CgiSocket(const CgiSocket& obj);
	CgiSocket& operator = (const CgiSocket& obj);
	CgiSocket* createCgiSocket(Request& req, std::string& response_buf);
...
namespace CgiMetaProcessors {
	...
	std::string get_remote_ident(const Request& req);
	...
	std::string get_server_protocol(const ServerConfig& conf);
